...
class BaseDataset(ABC):
    ...
    def adjust_weight_decay(self, current_size, total_budget, method):
        """
        ...
        if method == "predict":
            return float(self.get_predicted_weight_decay(current_size))
        
        if method == "optimal":
            # Ensure the returned value is a float
            return float(self.get_optimal_weight_decay(current_size, self.name))

        if method == "linear":
            decay = max_weight_decay - (max_weight_decay - min_weight_decay) * progress

        elif method == "exponential":
            decay = max_weight_decay * (min_weight_decay / max_weight_decay) ** progress

        elif method == "sigmoid":
            mid_point = total_budget / 2
            growth_rate = 10 / total_budget
            decay = min_weight_decay + (max_weight_decay - min_weight_decay) / (1 + np.exp(growth_rate * (current_size - mid_point)))

        elif method == "cosine":
            decay = min_weight_decay + 0.5 * (max_weight_decay - min_weight_decay) * (1 + np.cos(np.pi * progress))

        elif method == "polynomial":
            power = 2
            decay = (max_weight_decay - min_weight_decay) * (1 - progress) ** power + min_weight_decay
    ...
    def get_optimizer(self, model: Module, current_size=None, predicted_value=None) -> Optimizer:
        """
        Constructs the optimizer for this dataset according to the config and dynamically adjusts the weight decay.
        """
        method = self.config.get('weight_decay_strategy', 'linear')
        current_size = len(self.x_labeled)
        total_budget = self.budget

        if self.encoded:
            return self._construct_optimizer(model, self.config["optimizer_embedded"], current_size, total_budget, predicted_value)
        else:
            return self._construct_optimizer(model, self.config["optimizer"], current_size, total_budget, predicted_value)
