...
class ALGame(gym.Env):
    ...
    def tune_regularizers(self):
        def evaluate(params, n_iterations):
            # Save current model state
            current_state = self._get_internal_state()
            
            # Apply new regularizers
            for name, value in params.items():
                if name.startswith('reg_'):
                    setattr(self.classifier, name[4:], value)
            
            # Train for n_iterations
            reward = self._fit_classifier(epochs=int(n_iterations))
            
            # Restore original state
            self._set_internal_state(current_state)
            
            return -reward  # BOHB minimizes, so we negate the reward

        # Define the configuration space
        config_space = self.dataset.get_regularizer_config_space()
        
        # Initialize BOHB optimizer
        opt = BOHB(config_space, evaluate, max_budget=50, min_budget=10)
        
        # Run optimization
        logs = opt.optimize()
        
        # Access the best configuration from logs and convert it to a dictionary
        best_config = logs.best['hyperparameter'].to_dict()
        
        # Apply the best configuration to the model
        for name, value in best_config.items():
            if name.startswith('reg_'):
                setattr(self.classifier, name[4:], value)
        
        # Update optimizer with new weight decay
        for param_group in self.optimizer.param_groups:
            param_group['weight_decay'] = best_config['reg_weight_decay']
    ...
    def reset(self, *args, **kwargs) -> list:
        ...
        # Tune regularizers at the start
        self.tune_regularizers()
    ...
    def step(self, action: list[int]):
        ...
        # Check if we need to tune regularizers
        if self.added_images % (self.budget // 4) == 0:  # Tune every 25% of the budget
            self.tune_regularizers()
    ...
    def _get_internal_state(self):
        """
        Saves the current internal state of the model and optimizer.
        ...
        """
        model_state = copy.deepcopy(self.classifier.state_dict())
        optimizer_state = copy.deepcopy(self.optimizer.state_dict())
        return model_state, optimizer_state, self.current_test_loss, self.current_test_accuracy

    def _set_internal_state(self, state_tuple):
        """
        Restores the model and optimizer state from the provided tuple.
        Expects a tuple in the form:
            - model state dict
            - optimizer state dict
            - current test loss
            - current test accuracy
        """
        model_state, optimizer_state, test_loss, test_accuracy = state_tuple
        self.classifier.load_state_dict(model_state)
        self.optimizer.load_state_dict(optimizer_state)
        self.current_test_loss = test_loss
        self.current_test_accuracy = test_accuracy
...
class OracleALGame(ALGame):
    ...
    def step(self, *args, **kwargs):
        ...
        for id in chosen:
            # Fallback to margin sampling if no positive reward
            if scores[id] <= 0.0:
            ...
            # Recalculate weight decay based on the new labeled set size
            current_size = len(self.x_labeled)
            total_budget = self.budget
            new_weight_decay = self.dataset.adjust_weight_decay(current_size, total_budget, method=self.dataset.config['weight_decay_strategy'])

            # Update the optimizer with the new weight decay
            for param_group in self.optimizer.param_groups:
                param_group['weight_decay'] = new_weight_decay
            print(f"Updated weight decay to {new_weight_decay} at {current_size} labeled points")
